...
BANK_NAMES = ['СБЕР', 'Альфа Банк', 'Совкомбанк', 'Дом РФ', 'МКБ', 'ВТБ', 'ГПБ', 'ПСБ']
    ...
    yeet = sg.Window('Сверка Банка и ОСВ ver.1.0', background_color='#007bfb').Layout(
        [
            ...
            [sg.Text('Выберите тип банка', pad=(3,3), background_color='#007bfb'),
             sg.Listbox(values=BANK_NAMES, select_mode='extended', key='bank_name', size=(30, 8))],
...
def second_panel(files):
    ...
    yeet = sg.Window('Выберите файл или папку для обработки', layout, finalize=True)
    event, new_values = yeet.read()
    yeet.close()
    return (event, new_values)

def error_panel(error_desc):
    event = sg.popup(f'Возникла {error_desc}', 'Вы хотите повторить ввод данных?', title='Ошибка', custom_text = ('Да', 'Нет'))
    if event == 'Да':
        return True
    else:
        sys.exit()
...
def check_query_panel(query_dict, bank_name = '', filename = ''):
    ...
    if bank_name=='СБЕР':
        for key, value in query_dict.items():
            fields.append([sg.Text(key, size=(60, 7), tooltip=key), sg.InputText(default_text=value[0], key=f'{key}_query', size=(7,5),
                                                                    background_color='white', justification='center', text_color='black'),
                           sg.InputText(default_text=value[1], key=f'{key}_house', size=(7,5),
                                         background_color='white', justification='center', text_color='black')])
        layout = [
            [sg.Text(f'Текущий файл {filename}', size=(45, 5))],
            [sg.Text('Объект строительства',size=(60, 2)),  sg.Text('Очередь', size=(7, 2)), sg.Text('Дом', size=(7, 2))],
            [sg.Column(fields, size=(700, 700), scrollable=True, key="Column")],
            [sg.OK(), sg.Cancel()]
        ]
    elif bank_name!='ОСВ':
        for key, value in query_dict.items():
            fields.append([sg.Text(value[0], size=(15, 2)),
                           sg.Text(value[1], size=(15, 2)),
                           sg.InputText(default_text=value[0], key=f'{key}_query', size=(15, 2),
                                        background_color='white', justification='center', text_color='black'),
                           sg.InputText(default_text=value[1], key=f'{key}_house', size=(15, 2),
                                        background_color='white', justification='center', text_color='black')])
        layout = [[sg.Text(f'Текущий файл {filename}', size=(45, 5))],
                  [sg.Text('Текущая очередь', size=(15, 2)), sg.Text('Текущий дом', size=(15, 2)),
                   sg.Text('Новая очередь', size=(15, 2)),
                   sg.Text('Новый дом', size=(15, 2))],
                  [sg.Column(fields, size=(600, 700), scrollable=True, key="Column")],
                  [sg.OK(), sg.Cancel()]
                  ]
    else:
        for key, value in query_dict.items():
            fields.append([sg.Text(key, size=(60, 7), tooltip=key),
                           sg.InputText(default_text=value[0], key=f'{key}_query', size=(7, 5),
                                        background_color='white', justification='center', text_color='black'),
                           sg.InputText(default_text=value[1], key=f'{key}_house', size=(7, 5),
                                        background_color='white', justification='center', text_color='black')])
        layout = [
            [sg.Text('Группа', size=(60, 2)), sg.Text('Очередь', size=(7, 2)), sg.Text('Дом', size=(7, 2))],
            [sg.Column(fields, size=(700, 700), scrollable=True, key="Column")],
            [sg.OK(), sg.Cancel()]
        ]
...
def end_panel():
    event = sg.popup_auto_close(f'Сверка завершена', title='Завершение работы', auto_close_duration = 5)
    if event == 'OK':
        sys.exit()