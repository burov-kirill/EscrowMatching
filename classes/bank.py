...
class BankFile:
    ...
    def remove_na_rows(self, raw_bank_data, bank):
        ...
        for i in range(len(bank_data)):
            if sum(map(lambda x: x is None or x == '', bank_data.iloc[i])) > sum(map(lambda x: x is not None and x!='', bank_data.iloc[i])) or\
                    all(map(lambda x: type(x) == int, bank_data.iloc[i])):
    ...
    def create_check_correct_values(self, df):
        ...
        df = df[['Объект строительства', 'Номер ДДУ', 'Дата ДДУ', 'Очередь', 'Дом', 'Очередь (Договор)', 'Дом (Договор)']]
        df.columns = ['Объект строительства', 'Договор', 'Дата договора', 'Очередь (ОС)', 'Дом (ОС)', 'Очередь (Договор)', 'Дом (Договор)']
    ...
    def edit_DOM_contract(self, df, filename):
        ...
        for i in range(len(new_data)):
            result.setdefault(new_data['Депонент'][i], []).append(new_data['Новый договор'][i])
        for i in range(len(df)):
            if df['Договор'][i] == '':
                df['Договор'][i] = result.get(df['Контрагент'][i], ['бн'])[-1]
        return df
